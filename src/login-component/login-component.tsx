...
> {
  ...
  startPipe = (state: AuthInformation) => {
    ...
    const clientLibrary = new ClientLibrary(
      "http://127.0.0.1",
      "/api/v1/auth/thin/codes",
      5000,
      state
    );
      ...
      stream.on("data", async (data: any) => {
        ...
        switch (obj.method) {
          case "ClientLibrary.isClientRunning":
            returnMessage.result = clientLibrary.IsClientRunning(
              obj.params.sessionId
            );
            break;
          case "ClientLibrary.initSession":
            returnMessage.result = clientLibrary.InitSession(
              obj.params.sessionId
            );
            break;
          case "ClientLibrary.queryAuthorizationCode":
            returnMessage.result = await clientLibrary.QueryAuthorizationCode(
              obj.params.sessionId
            );
            break;
          case "ClientLibrary.queryGameAccountName":
            returnMessage.result = clientLibrary.QueryGameAccountName(
              obj.params.sessionId
            );
            break;
        }
      ...
      <Fragment>
        <span className={styles.login}>
          <FaUserCircle
            ...
            className={`${userName !== "" ? styles.online : ""} ${
              styles.loginLogo
            }`}
          />
          {userName !== "" ? (
